...
- The generated code (`generated/`, gitignored — see root `.gitignore`) has
  exactly one place it's imported from directly (`src/index.ts`), so if
  Prisma's generator output shape changes again, one file changes.
